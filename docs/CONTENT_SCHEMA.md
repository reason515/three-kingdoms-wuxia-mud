...
content/
  ...
  changan/
    du_jian/
      route.json                 # 已生成：19 个可执行事件，79 个选项
    ren_shuo/
...
```json
{
  ...
  "textVariants": [
    {
      "when": { "op": "hasFlag", "flag": "crossrun.attacker_saved" },
      "textKey": "event.changan.dujian.d02.attacker_saved"
    }
  ],
...
每个可达非结局事件必须有 1～6 个选项；普通事件目标仍为 2～3 个，复杂抉择 3～4 个，5～6 个仅用于终局或确有必要的复合局面。

`textVariants` 按顺序匹配第一个成立条件，用于人物命运、关系、跨周目见闻或自由状态造成的场景差异。基础 `textKey` 必须保持状态安全，不能无条件写入可能已经死亡、被捕或失散的人物。
...
```json
{
  ...
  "resolutions": [
    {
      "default": true,
      "resultTextKey": "result.choice.changan.dujian.d02.guard_xu.0",
      "effects": [
...
- 每条 `resolution` 应提供 `resultTextKey`，先叙述动作如何发生及人物如何回应，再进入下一事件；
...
- 关系与命运：`changeRelationship`、`setRelationship`、`addRelationshipTag`、`removeRelationshipTag`、`setCharacterFate`；
- 物品：`transferItem`、`setItemState`、`equipItem`、`unequipItem`、`consumeItem`、`confiscateVisibleItems`、`loseItemsByState`；
- 成长：`learnTechnique`、`addInsight`；
- 知识：`addKnowledge`；
- 调度：`scheduleEvent`、`cancelEvent`；
- 反馈：`emitFeedback`。

`changeResource` 可使用 `min` / `max` 限定结果，`changeInjury` 可使用 `minimum` / `maximum` 表达“至少重伤”等确定性结果。`hasItem` 可同时限定物品状态、持有者和是否装备。
...
```json
{
  ...
  "summaryVariants": [
    {
      "when": { "op": "compare", "path": "life", "cmp": "eq", "value": "dead" },
      "textKey": "ending.changan.dujian.protect.summary.dead"
    }
  ],
  "when": {},
  "excludeWhen": null,
...
5. 按顺序匹配 `summaryVariants`，生成符合生死、传承和自由状态的结局摘要；
6. 追加武学、装备、行止和江湖评语。
...
"texts": {
  ...
  "choice.changan.dujian.d02.guard_xu.intent": "需要动用内息；追敌会因此走远。",
  "result.choice.changan.dujian.d02.guard_xu.0": "你没有追箭手。短剑横过许简肩前……"
}
```
...
`tools/validate_content.py` 执行结构、操作符、引用和事件图校验；`tools/validate_du_jian_routes.py` 已通过最小运行时直接回放真实内容包中的五条代表路线。完整状态空间遍历仍需后续加入。
...
5. 运行内容校验器；
6. 运行代表路线回归测试；
7. 运行 `tools/validate_immersion.py`，检查即时结果、条件尾声、人物命运连续和选项提示；
8. 人工检查动作的站位、兵器、伤势、人物知识与状态变化是否一致；
9. 提交内容包、测试和版本号。
...
- 人物关系、物品实例、武学、知识和结局；
- Python 校验器和代表路线测试；
- MUD v0.1 运行时侧车状态：地点、等级、阅历、气血、体力、熟练、压力、训练次数、搜索记录、敌人和战斗；
- 剧情存档内嵌 `mud` 单局状态，人物武谱独立保存永久招式。

当前地点图和普通敌人定义位于 `docs/mud-engine.js`。验证核心循环后，应迁移为独立 `world` 内容包，建议类型为：

```text
world.locations[]
world.encounters[]
world.trainingRules[]
world.commandAliases{}
world.progression{}
```

地点记录至少包含稳定 ID、描述文本 key、相邻出口、NPC、一次性搜索结果、训练标签和有限遭遇 ID。战斗记录包含气力、架势、意图序列、奖励和失败后果。不得把单个剧情人物的特殊逻辑硬编码为通用战斗公式。
...
1. 杜缄、任朔剧情内容包和状态空间校验均已完成；
2. MUD v0.1 已实现共享探索、状态、行囊、武学、命令、有限训练和多回合战斗；
3. 将 `docs/mud-engine.js` 中的地点和遭遇迁移为独立 JSON world 内容包；
4. 增加 world Schema、引用校验、地点连通性和有限奖励校验；
5. 将 MUD 层获得的招式、伤势、物品和发现接入剧情条件；
6. 为任朔增加军阵、骑射与护列专属动作。
